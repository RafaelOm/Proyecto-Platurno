...
CREATE TABLE ALUMNO (USERNAME VARCHAR NOT NULL, APELLIDO1 VARCHAR, APELLIDO2 VARCHAR, CP VARCHAR, NOMBRE VARCHAR, PASSWORD VARCHAR NOT NULL, VALIDATIONCHAIN VARCHAR, DIRECCION VARCHAR, DNI VARCHAR, EMAIL_INSTITUCIONAL VARCHAR, EMAIL_PERSONAL VARCHAR, IDENTIFICADOR BIGINT, LOCALIDAD VARCHAR, MOVIL VARCHAR, PROVINCIA VARCHAR, TELEFONO VARCHAR, PRIMARY KEY (USERNAME))
CREATE TABLE USUARIO (USERNAME VARCHAR NOT NULL, PASSWORD VARCHAR NOT NULL, VALIDATIONCHAIN VARCHAR, IDENTIFICADOR BIGINT, PRIMARY KEY (USERNAME))
CREATE TABLE ASIGNATURA (Referencia VARCHAR NOT NULL, DTYPE VARCHAR(31), Caracter VARCHAR, Codigo INTEGER, Cred_Prac INTEGER, Creditos INTEGER, Curso VARCHAR, Duracion VARCHAR, Idiomas VARCHAR, Nombre VARCHAR, Ofertada VARCHAR, TITULACION_CodigoTitulacion VARCHAR, MENCION VARCHAR, PLAZAS VARCHAR, PRIMARY KEY (Referencia))
...
CREATE TABLE ENCUESTA (Id_Encuesta VARCHAR NOT NULL, Fecha_De_Envio DATE, Texto VARCHAR, EXPEDIENTE_Expediente VARCHAR, PRIMARY KEY (Id_Encuesta))
CREATE TABLE Expediente (Expediente VARCHAR NOT NULL, Activo VARCHAR, CreditosCF DOUBLE, CreditosFB DOUBLE, CreditosOB DOUBLE, CreditosOP DOUBLE, CreditosPE DOUBLE, CreditosSup DOUBLE, CreditosTF DOUBLE, N_Archivo VARCHAR NOT NULL, NotaMPr DOUBLE, ALUMNO_USERNAME VARCHAR, TITULACION_CodigoTitulacion VARCHAR, PRIMARY KEY (Expediente))
CREATE TABLE Gr_Asig (CURSO_ACT VARCHAR NOT NULL, OFERTA INTEGER, ASIG_Referencia VARCHAR NOT NULL, GROUP_ID VARCHAR NOT NULL, PRIMARY KEY (CURSO_ACT, ASIG_Referencia, GROUP_ID))
CREATE TABLE GRUPO (ID VARCHAR NOT NULL, Asignar VARCHAR, Curso INTEGER, Ingles VARCHAR NOT NULL, Letra VARCHAR, Letra1 VARCHAR, Plazas INTEGER, Turno VARCHAR NOT NULL, Visible VARCHAR, TITULO_CodigoTitulacion VARCHAR, PRIMARY KEY (ID))
...
CREATE TABLE MATRICULA (CURSO_ACADEMICO BIGINT NOT NULL, ESTADO VARCHAR, FECHA_MATRICULA VARCHAR, LISTADO_DE_ASIGNATURAS VARCHAR, NUEVO_INGRESO VARCHAR, NUM_ARCHIVO VARCHAR, TURNO_PREFERENTE VARCHAR, IDEXPEDIENTE_Expediente VARCHAR NOT NULL, PRIMARY KEY (CURSO_ACADEMICO, IDEXPEDIENTE_Expediente))
CREATE TABLE TITULACION (CodigoTitulacion VARCHAR NOT NULL, CREDITOS VARCHAR, NOMBRE VARCHAR, PRIMARY KEY (CodigoTitulacion))
CREATE TABLE SECRETARIA (USERNAME VARCHAR NOT NULL, PASSWORD VARCHAR NOT NULL, VALIDATIONCHAIN VARCHAR, APELLIDO VARCHAR, DNI VARCHAR, EMAIL VARCHAR, IDENTIFICADOR BIGINT, NOMBRE VARCHAR, PRIMARY KEY (USERNAME))
...
CREATE TABLE ALUMNO_Expediente (Alumno_USERNAME VARCHAR NOT NULL, expedientes_Expediente VARCHAR NOT NULL, PRIMARY KEY (Alumno_USERNAME, expedientes_Expediente))
CREATE TABLE ASIGNATURA_Gr_Asig (Asignatura_Referencia VARCHAR NOT NULL, CURSO_ACT VARCHAR NOT NULL, ASIG_Referencia VARCHAR NOT NULL, GROUP_ID VARCHAR NOT NULL, PRIMARY KEY (Asignatura_Referencia, CURSO_ACT, ASIG_Referencia, GROUP_ID))
...
CREATE TABLE ENCUESTA_Gr_Asig (encuestas_Id_Encuesta VARCHAR NOT NULL, CURSO_ACT VARCHAR NOT NULL, ASIG_Referencia VARCHAR NOT NULL, GROUP_ID VARCHAR NOT NULL, PRIMARY KEY (encuestas_Id_Encuesta, CURSO_ACT, ASIG_Referencia, GROUP_ID))
...
ALTER TABLE Expediente ADD CONSTRAINT FK_Expediente_ALUMNO_USERNAME FOREIGN KEY (ALUMNO_USERNAME) REFERENCES ALUMNO (USERNAME)
